            ...
            <Container>
                ...
                <View style={{paddingVertical: 20}}>
                    <AppText style={{marginBottom: 5, fontSize: 17}}>
                        Search related to {categoryList.find((value)=> value.id == category_id)?.name ?? "doctors"}
                    </AppText>
                    <AppText style={{fontSize: 14, color:"#1E1E1E80"}}>
                        {data.length} {categoryList.find((value)=> value.id == category_id)?.name ?? "doctors"} available.
                    </AppText>
                </View>
                <View style={styles.listCover}>
                    {data.map((value, idx)=>(
                        <View style={styles.card} key={idx}> 
                            <Image source={{uri: value.image}} style={styles.cardImage} resizeMode="cover" />
                            <View style={styles.cardContent}>
                                <AppText style={{marginBottom: 5,}} numberOfLines={1} ellipsizeMode="tail">Dr. {value.user.first_name} {value.user.last_name}</AppText>
                                <AppText>₦ {value.rate}/{value.per_rate === 0 && "hr"}{value.per_rate === 1 && "consult"}</AppText>
                            </View>
                            <NormalButtton onPress={()=>{navigateToDoctorDetails(value.id)}}>
                                View Details
                            </NormalButtton>
                        </View>
                    ))}
